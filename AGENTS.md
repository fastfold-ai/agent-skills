...
- Install for users: `npx skills add fastfold-ai/agent-skills`.
...
- **Skill directory:** `kebab-case` (e.g. `fastfold-fold-job`, `my-api-skill`)
...
- `python scripts/script_name.py <args> [--api-key KEY] [--base-url URL]`
...
- **Python:** Use `#!/usr/bin/env python3`; prefer `argparse` for CLI args; read API key from env (e.g. `os.environ.get("FASTFOLD_API_KEY")`) with optional `--api-key` override.
...
- **Errors:** Exit non-zero on failure; print a clear message to stderr (e.g. “Error: Unauthorized. Check FASTFOLD_API_KEY or --api-key.”).
...
Users must set `FASTFOLD_API_KEY` (or pass `--api-key`). The agent should ask the user to create/set the key before continuing if it is missing.
